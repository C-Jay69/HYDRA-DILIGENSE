...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Step 3: Run LLM analysis (optional, currently disabled)
      let llmFlags = [];
      if (useLLM) {
        try {
          llmFlags = await analyzeWithLLM(pdfText);
          console.log(`[3/4] LLM analysis: ${llmFlags.length} flags found`);
        } catch (llmError) {
          console.warn('[3/4] LLM analysis failed, using rule-based results only:', llmError);
          llmFlags = [];
        }
      } else {
        console.log('[3/4] LLM analysis skipped (disabled due to connection issues)');
      }
